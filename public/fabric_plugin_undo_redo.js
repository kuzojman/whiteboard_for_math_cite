...
fabric.Canvas.prototype._historyNext = function () {
	return function(){
    JSON.stringify(this.toDatalessJSON(this.extraProps));
    }}
...
fabric.Canvas.prototype._historyEvents = function () {
	return {
		'object:added': () => this._historySaveAction('add'),
		'object:removed': () => this._historySaveAction('remove'),
		'object:modified': () => this._historySaveAction('modify'),
		'object:skewing': () => this._historySaveAction('modify'),
	}
}
...
fabric.Canvas.prototype._historySaveAction = function (type) {
  // console.log(this.historyProcessing);
	...
	this.historyNextState = this._historyNext();
	this.fire('history:append', { type: type, json: json });
 ...
 * On the history
 */
fabric.Canvas.prototype.onHistory = function (type) {
	this.historyProcessing = false;

	this._historySaveAction(type);
}
