...
async function createGif(ab, url, id = false, params = false, noemit = false, originUrl)
{
  ...
  return getImgContentType(url).then(async t => {
      return fetch(url)
          ...
          .then(async  buffer => {
            ...
            if (t.indexOf('gif')!==-1) {
              return await createGif(buffer, url, id, params, noemit, originUrl).then(() =>{
                return true
              })
            } else {
...
window.insertImageOnBoard = async function (url, noemit=false, id=false, params=false){
  ...
  if (window.loaded[url]) {
    const blob = new Blob([window.loaded[url]["ab"]]);
