...
function selectTool(event) {
  ...
  if (selectedButton === currentButton && selectedButton) {
    if (
      !selectedButton.classList.contains('js-modal-trigger') &&
      !selectedButton.classList.contains('disable')
    ) {
    ...
    if (currentButton) {
      if (
        !currentButton.classList.contains('js-modal-trigger') &&
        !currentButton.classList.contains('disable')
      ) {
...
fabric.Canvas.prototype.toggleDragMode = function (state_ = false) {
  ...
  if (state_) {
    ...
    this.forEachObject(function (object) {
      ...
      object.selectable =
        object.prevSelectable !== undefined ? object.prevSelectable : object.selectable;
    });
    // Reset the cursor
...
function checkLoggedIn() {
  fetch(serverHostDebug + '/check_user_id/')
    .then((data) => data.json())
    .then((e) => {
...
function acceptAccess(e) {
  ...
  socket.emit('creator:response', {
    board_id: e.currentTarget.dataset.board,
...
function object_fit_apth(obj_) {
  ...
  if (object.path) {
    ...
    let bezierProcessedPath = [['M', ...bezierCurves[0][0]]];

    for (let i = 0; i < bezierCurves.length; i++) {
      bezierProcessedPath.push([
        'C',
        ...bezierCurves[i][1],
        ...bezierCurves[i][2],
        ...bezierCurves[i][3],
...
socket.on('connect', function () {
  ...
  socket.on('creator:request', (e) => {
    ...
    notifyPopup
      ...
      .addEventListener('click', acceptAccess, { once: true });
    notifyPopup
      .querySelector('#button-decline')
      .addEventListener('click', declineAccess, { once: true });
  });
  ...
  canvas.on('mouse:move', function (e) {
    if (bladeDown) {
      ...
      canvas._objects.forEach((item) => {
        ...
        if (
          cursorCoordinate.x > bound.left &&
          cursorCoordinate.x < bound.left + bound.width &&
          cursorCoordinate.y > bound.top &&
          cursorCoordinate.y < bound.top + bound.height
        ) {
          d.push(item);
          // console.log(item);
...
socket.on('coursour_disconected', function (user_id) {
  ...
  let index_of_clone_coursor = canvas._objects.findIndex(
    (item) => item.clone_id == `${user_id}_clone`
  );
...
fontColorInput2.addEventListener('click', () => {
  fontColorListWrapper2.classList.add('active');
});
...
function compare_path(f, s) {
  if (f.type != 'path' || s.type != f.type) return false;
  if (f.path.length != s.path.length) return false;
  if (!object_equals(f.path, s.path)) return false;
  return true;
}
...
function checkCursorClones(data) {
  ...
  if (
    ...
    data.cursorCoordinates.x > canvas.vptCoords.tr.x ||
    data.cursorCoordinates.y < canvas.vptCoords.tl.y ||
    data.cursorCoordinates.y > canvas.vptCoords.br.y
  ) {
    data.cursor = 'leave';
    ...
    if (data.cursorCoordinates.x > canvas.vptCoords.tr.x - 90)
      coords.x = canvas.vptCoords.tr.x - 90;
    if (data.cursorCoordinates.y < canvas.vptCoords.tl.y) coords.y = canvas.vptCoords.tl.y;
    if (data.cursorCoordinates.y > canvas.vptCoords.br.y - 90)
      coords.y = canvas.vptCoords.br.y - 90;
